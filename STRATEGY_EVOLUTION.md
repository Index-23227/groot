...
### 최종 시스템: VLM = "What", Skill = "How"

원래 GR00T VLA는 하나의 모델이 인식부터 제어까지 End-to-End로 처리하려 했다. 최종 시스템은 이와 정반대로, **역할을 명확히 분리**한다:

```
┌─────────────────────────────────────────────────┐
│  GPT-4o (인식 — "What")                         │
│                                                 │
│  입력: SAM2 crop 이미지들 + 자연어 명령          │
│  출력: pick 대상 번호, place 참조 번호, 위치     │
│                                                 │
│  ※ step5_actions도 출력하지만 코드에서 무시됨    │
└──────────────────┬──────────────────────────────┘
                   │ pick_index, place_ref_index,
                   │ place_position (u, v)
                   ▼
┌─────────────────────────────────────────────────┐
│  기하학 계산 (위치 특정 — "Where")               │
│                                                 │
│  Pick EE: SAM2 마스크 + Depth → 윗면 중점       │
│  Place EE: GPT-4o의 (u,v) → 원본 이미지 좌표    │
│  좌표 변환: 픽셀 → 카메라 3D → 로봇 좌표        │
└──────────────────┬──────────────────────────────┘
                   │ pick_robot (x,y,z)
                   │ place_robot (x,y,z)
                   ▼
...
│   5. 홈 복귀                                    │
│                                                 │
│  물체 타입(캔/블럭)은 z 높이로 판별 → 그립력 조절│
└─────────────────────────────────────────────────┘
```

이 구조는 SayCan 논문의 접근과 동일하다: **VLM이 "무엇을(what)" 결정하면, 로봇은 미리 정의된 primitive skill로 "어떻게(how)" 실행한다.** GPT-4o는 로봇 제어에 관여하지 않고, 로봇은 GPT-4o의 추론 과정을 알 필요가 없다.
